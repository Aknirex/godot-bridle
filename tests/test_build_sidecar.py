...
def test_linux_sidecar_target_has_no_extension(tmp_path: Path) -> None:
    target = build_sidecar.sidecar_target_path(tmp_path, "x86_64-unknown-linux-gnu")

    assert target.name == "bridle-sidecar-x86_64-unknown-linux-gnu"


def test_windows_sidecar_target_has_exe_extension(tmp_path: Path) -> None:
    target = build_sidecar.sidecar_target_path(tmp_path, "x86_64-pc-windows-msvc")

    assert target.name == "bridle-sidecar-x86_64-pc-windows-msvc.exe"
...
def test_sidecar_target_rejects_unsupported_platform(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="Unsupported sidecar target triple"):
        build_sidecar.sidecar_target_path(tmp_path, "aarch64-apple-darwin")

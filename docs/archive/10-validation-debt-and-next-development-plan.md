...
状态：`GITHUB_ENFORCEMENT_CONFIGURED_PENDING_SECRETS_AND_POST_CHANGE_GREEN_RUN`
...
- ✅ `main` 已要求 `Python 3.11`、`Python 3.12`、`Desktop TypeScript`、`Tauri Rust`
...
- 使用自定义 OpenAI-compatible embedding 端点执行最小请求；
...
状态：`IMPLEMENTATION_IN_PROGRESS_RELEASE_BLOCKING`
...
V4 已提升为 alpha 发布门禁。候选格式固定为 Windows x64 NSIS；在干净 Windows 11
Sandbox/VM 完成验证前不得创建 Linux 或 Windows alpha release。
...
D1-D4 完成后暂停功能扩张，依次完成 V1、V2、V3、V4 和 V5。全部阻塞项完成后才创建 `v0.1-alpha` release。
...
- 不在 V4 完成前创建任何平台的 alpha release 或宣称 Windows 正式支持；
- 新功能不得扩大 Alpha Exit Criteria，除非先更新计划文档并明确新的发布阻塞项。

## 6. 下一执行项

下一项工作固定为 **D5：Alpha 验证债务清零**。按 V1、V2、V3、V4、V5 顺序执行。

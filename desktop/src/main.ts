...
document.querySelectorAll<HTMLButtonElement>(".tab").forEach((tab) => {
  tab.onclick = () => {
    ...
    if (tab.dataset.page === "providers") void refreshProviders();
  };
});

$("#open-project").onclick = async () => {
  try {
    const result = await rpc("projects.open", { path: projectPath.value });
...
$("#generate-button").onclick = async () => {
  ...
  try {
    const result = await rpc("workflows.submit", {
...
$("#index-knowledge").onclick = async () => {
  ...
  try {
    ...
    const result = await rpc("knowledge.index", { project_path: path }) as {
      job_id: string;
    };
    await monitorJob(result.job_id);
...
$("#ask-knowledge").onclick = async () => {
  ...
  try {
    ...
    const result = await rpc("knowledge.ask", {
...
$("#cancel-job").onclick = async () => {
  if (appState.activeJob) await rpc("jobs.cancel", { job_id: appState.activeJob });
};
...
$("#save-provider").onclick = async () => {
  ...
  try {
    ...
    const result = await rpc("providers.save", {
...
async function initialize() {
  try {
    await waitForSidecar(invoke, 30_000, 250, (elapsedMs) => {
      $("#connection").textContent = `Starting local service… ${Math.floor(elapsedMs / 1_000)}s`;
    });
    const health = await rpc("system.health") as Json;
...
async function monitorJob(jobId: string): Promise<void> {
  ...
  await rpc("jobs.subscribe", { job_id: appState.activeJob, after_sequence: 0 });
}
...
async function refreshProviders(): Promise<void> {
  const providers = await rpc("providers.list") as Json[];
  ...
  providers.forEach((provider) => {
    ...
    test.onclick = async () => {
      try {
        output.textContent = JSON.stringify(await rpc("providers.test", {
...
async function refreshKnowledgeStatus(): Promise<void> {
  const status = await rpc("knowledge.status", {
    project_path: selectedProjectPath(),
  }) as Json;
